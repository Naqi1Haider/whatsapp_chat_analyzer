...
def preprocess(data):
    ...
    df['message_date'] = pd.to_datetime(df['message_date'], errors='coerce', format='%d/%m/%Y, %I:%M\u202f%p')

    # Drop rows with NaT (Not a Time) values
    df = df.dropna(subset=['message_date'])

    # Format the datetime column to display time in AM/PM format
    ...
    for message in df['user_message']:
        ...
        if len(entry) > 1:  # Check if username exists
            users.append(entry[1])
            messages.append(" ".join(entry[2:]))
        else:
            users.append('group_notification')
            messages.append(entry[0].strip("- "))  # Remove leading dash if present
